				...
				do
				{
					...
					switch (type)
					{
						case Component::RENDER:
							if (strcmp(componentNode->FirstChild()->ToElement()->Name(), "Asset") == 0)
							{
								Asset* asset = 0;
								const XMLElement* assetElement = componentNode->FirstChild()->ToElement();
								string assetId(assetElement->Attribute("id"));
						...
						case Component::PHYSICS:
							if (strcmp(componentNode->FirstChild()->ToElement()->Name(), "Physics") == 0)
							{
								...
								component = new PhysicsComponent(componentId, bodyType, density, enabled);
							}
							break;
					}
					if (component)
